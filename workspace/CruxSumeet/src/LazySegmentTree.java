 ...
 class LazySegmentTree
{
     ...
     LazySegmentTree(int max){
    	this.MAX = max;
    	tree = new int[2*MAX+1];
    	lazy =  new int[2*MAX+1];
    }
    ...
    /* A recursive function that constructs Segment Tree for
      ...
      tree st. */
    void constructSTUtil(int arr[], int low, int high, int pos)
    {
    	if(low == high){
            tree[pos] = arr[low];
            return;
    ...
    public static void main(String args[])
    {
        ...
        tree.updateRange(n, 0, 3, 3);
        tree.updateRange(n, 0, 3, 1);
        tree.updateRange(n, 0, 0, 2);
